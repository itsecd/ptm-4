...
def results():
    ...
    if computer_choice == "Snake":
        if user_choice == "s" or user_choice == "snake":
            ...
            logging.info(f"Player has drawn {user_choice} - {computer_choice},"
            ...
            logging.info(f"Player have lost {user_choice} - {computer_choice},"
                         f"Player`s point {user_points}, Computer`s points {computer_points}")
            ...
            logging.info(f"Player has won {user_choice} - {computer_choice},"
                         f"Player`s point {user_points}, Computer`s points {computer_points}")
        else:
        ...
        if user_choice == "s" or user_choice == "snake":
            ...
            print(yellow("AWWW...!!"), emojize(":neutral_face:"), yellow("\nWATER IS INCREASED :/"))
            logging.info(f"Player has draw {user_choice} - {computer_choice},"
                         f"Player`s point {user_points}, Computer`s points {computer_points}")
        elif user_choice == "g" or user_choice == "gun":
            print(red("NOOO...!!"), emojize(":crying_face:"), red("\nYOUR GUN SANK INTO THE WATER :("))
            computer_points += 1  # Incrementing computer_points by one
            logging.info(f"Player has lost {user_choice} - {computer_choice},"
                         f"Player`s point {user_points}, Computer`s points {computer_points}")
        else:
            print("!!कृपया सही विकल्प चुनें!!")  # Error message in 'HINDI' for invalid choice
            flag = 1  # Changing flag value to iterate the loop again in same chance
...
def replay_game():
    ...
    while 1:  # infinite loop till the user enters a valid choice
        ...
        if replay == "y" or replay == "yes":
            logging.info("Player want to replay")
            break  # breaking infinite loop after getting valid input
        elif replay == "n" or replay == "no":
            logging.info("Player didn`t want to replay, game was exited")
            break  # breaking infinite loop after getting valid input
        else:
...
while 1:  # Infinite loop to play the game as many times as the user wants
    ...
    try:
        ...
        print(red('Please enter only natural number', 'bold'))
        logging.error(f"Invalid value (chances) from player {err}")
        continue
    ...
    if computer_points > user_points:
        ...
        logging.info(f"Final, player was lost, player: {user_points} - Com: {computer_points}")
    elif user_points > computer_points:
        print(green("\n\t\t\tHURRAH !!"), emojize(":smiling_face_with_sunglasses:"), green("YOU WON THE GAME"))
        logging.info(f"Final, player was won, player: {user_points} - Com: {computer_points}")
    else:
        print(yellow("\n\t\t\t!! TIE !!"), emojize(":disappointed_face:"), yellow("!! TRY AGAIN !!"))
        logging.info(f"Final, player was drawn, player: {user_points} - Com: {computer_points}")
    replay = ""
    ...
    if replay == "y" or replay == "yes":
        ...
        logging.info("Game was restarted")
