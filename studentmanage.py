...
entry_font = ('Garamond', 12)
NAME_FILE = 'SchoolManagement.db'

# Connecting to the Database where all information will be stored
try:
    connector = sqlite3.connect(NAME_FILE)
    logging.info(f"File <{NAME_FILE}> was created successfully")
...
def reset_fields():
    ...
    try:
        for i in ['name_strvar', 'email_strvar', 'contact_strvar', 'gender_strvar', 'stream_strvar']:
            exec(f"{i}.set('')")
            logging.info(f"The data {i} was reset successfully")
        dob.set_date(datetime.datetime.now().date())
    except Exception as err:
        logging.error(f"Error reset all data:{err}")


def reset_form():
    """
    Function the reset data
    """
    global tree
    try:
        tree.delete(*tree.get_children())
        reset_fields()
        logging.info(f"The data {tree.get_children()} was reset successfully")
    except Exception as err:
        logging.error(f"Error reset data {tree.get_children()} :{err}")
...
def display_records():
    ...
    try:
        ...
        for records in data:
            ...
            logging.info(f"The data {records} was recorded successfully")

    except Exception as err:
        logging.error(f"Error record data:{err}")
...
def add_record():
    ...
    try:
        if not name or not email or not contact or not gender or not DOB or not stream:
            ...
            logging.info(f"The data has not been filled in completely")
        else:
            try:
                connector.execute(
                ...
                logging.info(f"The data <{name}> has been added successfully")
                reset_fields()
                display_records()
            except ValueError as err:
                mb.showerror('Wrong type',
                             'The type of the values entered is not accurate. Pls note that the contact field can '
                             'only contain numbers')
                logging.error(f"Error wrong type:{err}")
    except Exception as err:
        logging.error(f"Error add data:{err}")
...
def remove_record():
    ...
    try:
        if not tree.selection():
            ...
            logging.info(f"The data <{tree.selection()}> is not select to remove")
        else:
        ...
        logging.error(f"Error remove data:{err}")
...
def view_record():
    ...
    try:
        if not tree.selection():
            ...
            logging.info(f"The data <{tree.selection()}> is not select to view")
        else:
            current_item = tree.focus()
            values = tree.item(current_item)
            selection = values["values"]
        ...
        logging.error(f"Error record data to view:{err}")
...
Label(main, text="SCHOOL MANAGEMENT SYSTEM", font=head_label_font, bg='SpringGreen').pack(side=TOP, fill=X)
...
Label(left_frame, text="Name", font=label_font, bg=lf_bg).place(relx=0.375, rely=0.05)
Label(left_frame, text="Contact Number", font=label_font, bg=lf_bg).place(relx=0.175, rely=0.18)
Label(left_frame, text="Email Address", font=label_font, bg=lf_bg).place(relx=0.2, rely=0.31)
Label(left_frame, text="Gender", font=label_font, bg=lf_bg).place(relx=0.3, rely=0.44)
Label(left_frame, text="Date of Birth (DOB)", font=label_font, bg=lf_bg).place(relx=0.1, rely=0.57)
Label(left_frame, text="Stream", font=label_font, bg=lf_bg).place(relx=0.3, rely=0.7)
...
Entry(left_frame, width=19, textvariable=stream_strvar, font=entry_font).place(x=20, rely=0.75)
OptionMenu(left_frame, gender_strvar, 'Male', "Female").place(x=45, rely=0.49, relwidth=0.5)
dob = DateEntry(left_frame, font=("Arial", 12), width=15)
dob.place(x=20, rely=0.62)
Button(left_frame, text='Submit and Add Record', font=label_font, command=add_record, width=18).place(relx=0.025,
                                                                                                      rely=0.85)
...
Label(right_frame, text='Students Records', font=head_label_font, bg='DarkGreen', fg='LightCyan').pack(side=TOP, fill=X)
